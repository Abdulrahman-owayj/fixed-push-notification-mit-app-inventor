...
public class NotificationStyle extends AndroidNonvisibleComponent implements OnDestroyListener {
    ...
    private int receiverFlagsCompat() {
        return Build.VERSION.SDK_INT >= 33 ? Context.RECEIVER_NOT_EXPORTED : 0;
    }

    private void registerReceiverCompat(BroadcastReceiver receiver, IntentFilter filter) {
        if (Build.VERSION.SDK_INT >= 33) {
            activity.registerReceiver(receiver, filter, receiverFlagsCompat());
        } else {
            activity.registerReceiver(receiver, filter);
        }
    }

    private int immutableUpdateCurrentFlags() {
    ...
    private int mutableUpdateCurrentFlags() {
        int flags = PendingIntent.FLAG_UPDATE_CURRENT;
    ...
    public NotificationStyle(final ComponentContainer container) {
        ...
        registerReceiverCompat(receiver, new IntentFilter("MUSIC_FAVORITE"));
        registerReceiverCompat(receiver, new IntentFilter("MUSIC_PAUSE"));
        registerReceiverCompat(receiver, new IntentFilter("MUSIC_PLAY"));
        registerReceiverCompat(receiver, new IntentFilter("MUSIC_PREVIOUS"));
        registerReceiverCompat(receiver, new IntentFilter("MUSIC_NEXT"));
        registerReceiverCompat(messageBroad, new IntentFilter("MESSAGE_REPLY"));
        registerReceiverCompat(progressBroad, new IntentFilter("BUTTON1"));
        registerReceiverCompat(progressBroad, new IntentFilter("BUTTON2"));
    ...
    private void sendNotification(final String title, final String subtitle, final boolean bigtext,
        ...
        for (final String button : listButtons) {
            ...
            registerReceiverCompat(actionBroad, new IntentFilter(nameButton));
        }
    ...
    private void notificationMessage(final String group, final String message, final String sender,
        ...
        replyPendingIntent = PendingIntent.getBroadcast(context, 0, replyIntent, mutableUpdateCurrentFlags());
